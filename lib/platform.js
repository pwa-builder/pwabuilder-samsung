...
function Platform(packageName, platforms) {
  ...
  self.fetchApkLink = function(bytes, sourceDir, platformDir, manifest) {
    const headers = {
    ...
    .then(response => {
      ...
      var obj_package_name = object.packageName;
      var version = object.version;
      var token = object.token;
      var link = self.webapkUrl + token + "/" + version + "/" + obj_package_name + ".apk";
      return link;
    }).then(async link => {
  ...
  self.create = function (manifest, rootDir, chromeVersion, callback) {
    ...
    const platformDir = self.getOutputFolder(rootDir);
    const sourceDir = path.join(platformDir, 'source');
    ...
    .then(function () {
      ...
      return fetch(imageLink)
        ...
        .then(images => images.arrayBuffer())
        .then(buffer => {
          ...
          const scopes = manifest.content.scope;
          ...
          return bytes;
        }).then(bytes => self.fetchApkLink(bytes, sourceDir, platformDir, manifest))
     })
    .nodeify(callback);
  };
}
