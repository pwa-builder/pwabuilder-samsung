...
function Platform(packageName, platforms) {
   ...
   */
  self.create = function (manifest, chromeVersion) {
    ...
    return fetch(self.webapkApiUrl, {
      ...
      .then(response => {
        ...
        var link = self.webapkUrl + token + "/" + version + "/" + self.apkName + ".webapk." + token + ".apk";
