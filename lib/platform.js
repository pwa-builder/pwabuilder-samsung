...
function Platform (packageName, platforms) {
  ...
  self.addManifestIcon = function (manifest, fileName, size) {
    ...
    self.webapk = function(manifest, chromeVersion, webapkUrl) {
    const headers = {
    ...
    const requester_application_package = "com.sec.android.app.sbrowser.beta";
    ...
    .then(response => {
      ...
      //get(token, version, package_name);
